...
/**
* Gets the StudentsObj (see documentation.gs for type) by pulling information from the Teams sheet and 
* from the responses. 
*
* If a student doesn't have a asurite ID on the teams sheet, then they get a generic ID like so: `unknown1`.
*
* DEV NOTE: This could be made more efficient. Right now the part slowing it down is `getStudentGrades`. It could
* likely use the looping already done through the responses to get that information.
*
...
function getStudentsObj() {
  if (studentsObj === null) {
    ...
    const asuRiteQuestionFormItem = formItems.find(function (item) {
      return item.getTitle() === questionTitles.asuId;
    });
        ...
        for (let j = 0; j < studentItemResponses.length; j++) {
          ...
          if (studentItemResponses[j].getResponse() !== null) {
            ...
            if (formIdsMaps.gradingFormItemIds[responseItemId] !== undefined) {
              ...
              studentsObj[studentIdBeingGraded].peerGrades.push(studentItemResponses[j].getResponse());
              ...
              studentsObj[studentIdBeingGraded].peerComments.push(studentItemResponses[j].getResponse());
            }
          }
